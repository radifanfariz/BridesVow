...
const Cover = ({contents,isPlayerReady,playMusicCallback}) => {
    ...
    useEffect(() => {
        if(isPlayerReady){
            setBtnStyle({
                title:"Buka Undangan",
                style:"bg-[#2a5d87] w-[185px] h-[36px] hover:opacity-25 p-2 m-5 mt-4 rounded-full text-white border-[1px] border-black",
            })
        ...
        <header className={open}>
            ...
            <div className="flex flex-col h-full min-h-screen justify-center">
                ...
                <div className="flex z-10 justify-center items-center pt-5">
                    <div className="font-bold text-7xl text-black text-center font-[playfair]">{`${contents.cewekName} & ${contents.cowokName}`}</div>
                </div>
